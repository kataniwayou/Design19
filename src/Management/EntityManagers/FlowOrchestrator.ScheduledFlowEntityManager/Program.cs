...
builder.Services.AddSingleton<ITelemetryProvider>(provider =>
    new OpenTelemetryProvider("FlowOrchestrator.ScheduledFlowEntityManager"));
builder.Services.AddSingleton<ILogger>(provider =>
    new OpenTelemetryLogger(
        provider.GetRequiredService<ITelemetryProvider>(),
        "FlowOrchestrator.ScheduledFlowEntityManager"));

// Add MongoDB
builder.Services.AddMongoDb(builder.Configuration);
builder.Services.AddMongoRepository<ScheduledFlowEntity>("scheduledflows");
builder.Services.AddMongoRepository<FlowEntity>("flows");
builder.Services.AddMongoRepository<SourceAssignmentEntity>("sourceassignments");
...
builder.Services.AddSingleton<IValidator<ScheduledFlowEntity>, ScheduledFlowValidator>();
builder.Services.AddSingleton<ScheduledFlowService>();
