...
builder.Services.AddSingleton<ITelemetryProvider>(provider =>
    new OpenTelemetryProvider("FlowOrchestrator.DestinationAssignmentEntityManager"));
builder.Services.AddSingleton<ILogger>(provider =>
    new OpenTelemetryLogger(
        provider.GetRequiredService<ITelemetryProvider>(),
        "FlowOrchestrator.DestinationAssignmentEntityManager"));

// Add MongoDB
builder.Services.AddMongoDb(builder.Configuration);
builder.Services.AddMongoRepository<DestinationAssignmentEntity>("destinationassignments");
builder.Services.AddMongoRepository<DestinationEntity>("destinations");
builder.Services.AddMongoRepository<ExporterEntity>("exporters");
...
builder.Services.AddSingleton<IValidator<DestinationAssignmentEntity>, DestinationAssignmentValidator>();
builder.Services.AddSingleton<DestinationAssignmentService>();
