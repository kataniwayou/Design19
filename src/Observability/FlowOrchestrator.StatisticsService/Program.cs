...
using FlowOrchestrator.StatisticsService;
...
builder.Services.AddSingleton<ITelemetryProvider>(provider =>
    new FlowOrchestrator.Infrastructure.Common.Telemetry.OpenTelemetryProvider("StatisticsService"));

// Configure OpenTelemetry
builder.Logging.AddOpenTelemetry(options =>
{
    options.SetResourceBuilder(ResourceBuilder.CreateDefault().AddService("StatisticsService"));
});

// Add application services
builder.Services.AddSingleton<FlowOrchestrator.StatisticsService.Services.StatisticsService>();
