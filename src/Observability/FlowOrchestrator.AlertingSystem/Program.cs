...
builder.Services.AddSingleton<ITelemetryProvider>(provider =>
    new FlowOrchestrator.Infrastructure.Common.Telemetry.OpenTelemetryProvider("AlertingSystem"));

// Configure OpenTelemetry
builder.Logging.AddOpenTelemetry(options =>
{
    options.SetResourceBuilder(ResourceBuilder.CreateDefault().AddService("AlertingSystem"));
});
