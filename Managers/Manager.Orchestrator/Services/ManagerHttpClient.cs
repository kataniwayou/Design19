...
public class ManagerHttpClient : IManagerHttpClient
{
        ...
        IConfiguration configuration)
    {
        ...
        _resilientPolicy = Policy
            ...
            .WaitAndRetryAsync(
                ...
                {
                    _logger.LogWarning("HTTP request retry {RetryCount} after {Delay}ms. Reason: {Reason}",
    ...
    public async Task<OrchestratedFlowEntity?> GetOrchestratedFlowAsync(Guid orchestratedFlowId)
    {
        ...
        _logger.LogInformation("Retrieving orchestrated flow. OrchestratedFlowId: {OrchestratedFlowId}, Url: {Url}",
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.LogInformation("Successfully retrieved orchestrated flow. OrchestratedFlowId: {OrchestratedFlowId}, WorkflowId: {WorkflowId}, AssignmentCount: {AssignmentCount}",
            ...
            {
                _logger.LogWarning("Orchestrated flow not found. OrchestratedFlowId: {OrchestratedFlowId}",
                    orchestratedFlowId);
                return null;
            }

            _logger.LogError("Failed to retrieve orchestrated flow. OrchestratedFlowId: {OrchestratedFlowId}, StatusCode: {StatusCode}, Reason: {Reason}",
        ...
        {
            _logger.LogError(ex, "Error retrieving orchestrated flow. OrchestratedFlowId: {OrchestratedFlowId}",
                orchestratedFlowId);
            throw;
        }
    }
    ...
    {
        _logger.LogInformation("Retrieving step manager data. WorkflowId: {WorkflowId}",
            workflowId);
        ...
        try
        {
            ...
            _logger.LogInformation("Retrieving workflow to get step IDs. WorkflowId: {WorkflowId}, BaseUrl: {BaseUrl}, FullUrl: {Url}",
            ...
            {
                _logger.LogError("Failed to retrieve workflow. WorkflowId: {WorkflowId}, StatusCode: {StatusCode}, Reason: {Reason}",
            ...
            {
                _logger.LogWarning("Workflow not found or has no steps. WorkflowId: {WorkflowId}",
                    workflowId);
                return new StepManagerModel();
            }

            _logger.LogDebug("Retrieved workflow with {StepCount} steps. WorkflowId: {WorkflowId}, StepIds: {StepIds}",
            ...
            var stepTasks = workflow.StepIds.Select(async stepId =>
            {
                ...
                _logger.LogWarning("Failed to retrieve step. StepId: {StepId}, StatusCode: {StatusCode}",
            ...
            _logger.LogInformation("Successfully retrieved step manager data. WorkflowId: {WorkflowId}, StepCount: {StepCount}, ProcessorCount: {ProcessorCount}, NextStepCount: {NextStepCount}",
        ...
        {
            _logger.LogError(ex, "Error retrieving step manager data. WorkflowId: {WorkflowId}",
                workflowId);
            throw;
        }
    }
    ...
    public async Task<AssignmentManagerModel> GetAssignmentManagerDataAsync(List<Guid> assignmentIds)
    {
        ...
        {
            _logger.LogInformation("No assignment IDs provided, returning empty assignment manager model");
            return model;
        }

        var baseUrl = _configuration["ManagerUrls:Assignment"];

        _logger.LogInformation("Retrieving assignment manager data. AssignmentIds: {AssignmentIds}",
        ...
        try
        {
            ...
            var assignmentTasks = assignmentIds.Select(async assignmentId =>
            {
                ...
                _logger.LogWarning("Failed to retrieve assignment. AssignmentId: {AssignmentId}, StatusCode: {StatusCode}",
            ...
            _logger.LogInformation("Successfully retrieved assignment manager data. AssignmentCount: {AssignmentCount}, TotalEntities: {TotalEntities}",
        ...
        {
            _logger.LogError(ex, "Error retrieving assignment manager data. AssignmentIds: {AssignmentIds}",
                string.Join(",", assignmentIds));
            throw;
        }
    }
    ...
    private async Task<AssignmentModel?> ProcessAssignmentEntityAsync(Guid entityId)
    {
        try
        {
            ...
            _logger.LogWarning("Entity not found in Address or Delivery managers. EntityId: {EntityId}", entityId);
            return null;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing assignment entity. EntityId: {EntityId}", entityId);
            return null;
        }
    }
    ...
    private async Task<AddressModel?> TryGetAddressEntityAsync(Guid entityId)
    {
        ...
        {
            _logger.LogDebug(ex, "Failed to retrieve entity as address. EntityId: {EntityId}", entityId);
            return null;
        }
    }
    ...
    private async Task<DeliveryModel?> TryGetDeliveryEntityAsync(Guid entityId)
    {
        ...
        {
            _logger.LogDebug(ex, "Failed to retrieve entity as delivery. EntityId: {EntityId}", entityId);
            return null;
        }
    }
    ...
    public async Task<string> GetSchemaDefinitionAsync(Guid schemaId)
    {
        ...
        _logger.LogDebug("Retrieving schema definition. SchemaId: {SchemaId}, Url: {Url}",
            schemaId, url);
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.LogDebug("Successfully retrieved schema definition. SchemaId: {SchemaId}",
                    schemaId);

                return schemaEntity?.Definition ?? string.Empty;
            }

            _logger.LogWarning("Failed to retrieve schema definition. SchemaId: {SchemaId}, StatusCode: {StatusCode}",
        ...
        {
            _logger.LogError(ex, "Error retrieving schema definition. SchemaId: {SchemaId}",
                schemaId);
            return string.Empty;
        }
    }
